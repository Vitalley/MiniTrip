...
void screen1(void)
{
	...
	OLED_mid(0,6,impavr/50000,0x3C,3);OLED_dot_mid(33,6,0x3C);OLED_mid(36,6,(impavr/5000)%10,0x3C,1);
...
void screen2(void)
{
	scrn=2;// Литры в час, всего израсходовано, осталось пробега
	PutStr_mid(2,0,"Fuel const",0x3C);
...
void screen4(void)
{
	...
	PutStr_mid(0,4,"Fuel",0x3C);OLED_mid(84,4,impavr/50000,0x3C,3);// Израсходовано топлива
			OLED_dot_mid(116,4,0x3C); OLED_mid(120,4,(impavr/5000)%10,0x3C,1);
...
void screen7(void)
{
	...
	//unsigned int rash=(200*(unsigned long int)fuel)/speed;
	unsigned int avrrash=(impavr/50)/trip;
	PutStr_mid(2,0,"Fuel tank",0x3C);
	OLED_temper_big(16, 2,fueltank, 0x3C); //Остаток топлива в баке
	PutStr_mid(95,3,"ltr",0x3C);
	OLED_mid(0,6,impavr/50000,0x3C,3);//Израсходовано топлива
	OLED_dot_mid(33,6,0x3C);
	OLED_mid(36,6,(impavr/5000)%10,0x3C,1);
	PutChar_mid(46,6,'l',0x3C);
	OLED_mid(74,6,(10*(unsigned long int)fueltank)/avrrash,0x3C,3);// Остаток пробега на основе мгновенного расхода топлива
	PutStr_mid(106,6,"km",0x3C);
}
